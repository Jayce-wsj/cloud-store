...
func (q *SB) Release() {
	...
	if q.duplicateUpdateValues != nil {
		if q.duplicateUpdateValues != q.values {
			q.duplicateUpdateValues.Release()
		}
...
func (q *SB) processUpdateValues(vals *Values) *bytes.Buffer {
	...
	for i, k := range vals.Keys {
		...
		if iv, ok := vals.Vals[i].(IncVal); ok {
			if iv.BaseField == "" {
				placeholder.WriteString(k)
			} else {
			...
			placeholder.WriteString(I64toA(iv.Val))
		} else {
			placeholder.WriteString("?")
...
func (q *SB) Limit(count int64, offset ...int64) *SB {
	if len(offset) > 0 {
		q.limit = I64toA(offset[0]) + "," + I64toA(count)
	} else {
		q.limit = "0," + I64toA(count)
	}
	return q
}
...
func (q *SB) Exec(args ...interface{}) *SBResult {
	...
	if q.fullSQL {
		...
		if q.tx != nil {
			ret, sbRet.Err = q.tx.Exec(sqlStr)
		} else {
			ret, sbRet.Err = q.db.Exec(sqlStr)
		}
	} else {
		if q.tx != nil {
			ret, sbRet.Err = q.tx.Exec(sbRet.Sql, append(q.args, args...)...)
		} else {
			ret, sbRet.Err = q.db.Exec(sbRet.Sql, append(q.args, args...)...)
		}
		if sbRet.Err != nil {
			return sbRet
		}
...
func (q *SB) QueryOne(args ...interface{}) (OneRow, error) {
	...
	if q.debug {
		//xlog.Debugf("SQL prepare statement: %s, Params: %+v", s, args)
	}
	if q.tx == nil {
